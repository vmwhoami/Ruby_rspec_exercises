...
vowels = ["a", "e", "i", "o", "u"]
	words.map.with_index do |word_1,indx_1|
		...
		if vowels.all?{|el| double.include?(el)} && indx_2 > indx_1 # This is an important concept Remember this
		 double
		end
...
class String
    ...
    def substrings(length = nil)
        ar = []
				self.each_char.with_index do |char_1,index_1|
					str = char_1
					ar << char_1
            self.each_char.with_index do |char_2, index_2|
							if index_1<index_2
								str += char_2 
								ar << str
							end
    ...
    def caesar_cipher(num)
		alpha = ("a".."z").to_a
		self.split("").map {|char| alpha[(alpha.index(char) + num ) % 26]}.join("")
    end
end
