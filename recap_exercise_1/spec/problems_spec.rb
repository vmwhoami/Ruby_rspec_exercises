...
describe 'all_vowel_pairs' do
  it 'should return all pairs of words that contain every vowel' do
    expect(all_vowel_pairs(%w[goat action tear impromptu tired europe])).to eq(['action europe', 'tear impromptu'])
    expect(all_vowel_pairs(%w[upper goalie unstoppable cranky terrible])).to eq(['upper goalie', 'goalie unstoppable', 'unstoppable terrible'])
